...
export class DominoGame {
  ...
  getRoundWinnerInfo() {
    ...
    // Verificar se alguém ficou sem peças (batida normal)
    let winnerPlayerIndex = this.players.findIndex((p) => p.getPieces().length === 0);

    if (winnerPlayerIndex !== -1) {
      // Vitória por batida - jogador ficou sem peças
      const winnerTeamIndex = winnerPlayerIndex % 2;
      return {
        teamIndex: winnerTeamIndex,
        playerName: this.players[winnerPlayerIndex].name,
        winType: 'batida',
        teamScores: [0, 0] // Não há contagem de pontos na batida
      };
    }

    // Jogo fechado - todos passaram a vez, calcular pela menor pontuação
    let teamScores = [0, 0];
    let playerScores = [];
    
    this.players.forEach((player, index) => {
      const teamIndex = index % 2;
      const playerPoints = player.getPieces().reduce((sum, piece) => sum + piece.left + piece.right, 0);
      playerScores[index] = playerPoints;
      teamScores[teamIndex] += playerPoints;
    });

    // Determinar dupla vencedora (menor pontuação)
    const winnerTeamIndex = teamScores[0] <= teamScores[1] ? 0 : 1;
    
    // Encontrar o jogador da dupla vencedora com menor pontuação individual
    let bestPlayerIndex = winnerTeamIndex; // Começa com o primeiro jogador da dupla
    if (winnerTeamIndex === 0) {
      // Dupla 1: jogadores 0 e 2
      bestPlayerIndex = playerScores[0] <= playerScores[2] ? 0 : 2;
    } else {
      // Dupla 2: jogadores 1 e 3  
      bestPlayerIndex = playerScores[1] <= playerScores[3] ? 1 : 3;
    }

    return {
      teamIndex: winnerTeamIndex,
      playerName: this.players[bestPlayerIndex].name,
      winType: 'fechado',
      teamScores: teamScores,
      playerScores: playerScores,
      pointDifference: Math.abs(teamScores[0] - teamScores[1])
    };
  }
